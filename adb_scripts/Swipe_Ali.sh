...
if [[ -x "/d/opt64/scrcpy/adb.exe" ]]
then
    ADBBIN=/d/opt64/scrcpy/adb.exe
elif [[ -x "/c/opt64/scrcpy/adb.exe" ]]
then
    ADBBIN=/c/opt64/scrcpy/adb.exe
...
elif [[ -n  `which adb 2>/dev/null` ]]
then
    echo "default adb found"
    ADBBIN=adb
fi

if [[ -x "/d/opt64/scrcpy/scrcpy.exe" ]]
then
    SCRCPYBIN=/d/opt64/scrcpy/scrcpy.exe
elif [[ -x "/c/opt64/scrcpy/scrcpy.exe" ]]
then
    SCRCPYBIN=/c/opt64/scrcpy/scrcpy.exe
elif [[ -n  `which scrcpy.exe 2>/dev/null` ]]
then
    echo "default scrcpy.exe found"
    SCRCPYBIN=scrcpy.exe
fi
